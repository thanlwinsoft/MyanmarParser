...
* Copyright: 2005,2009 http://www.thanlwinsoft.org
*
...
{  
   ...
   *     ဢူႛꩰ
   */
  public void test_lba_kht_01()
  {
    ClusterProperties cp = myParser.getNextSyllable(STRING_K.toCharArray(), 0);
    assertEquals(cp.toString(), 4, cp.getEnd() );
    assertEquals(cp.toString(), MyanmarParser.MyPairStatus.MY_PAIR_EOL,
                 cp.getBreakStatus());
  }
   ...
   * 
   */
  public void test_lba_mnw_01()
  {
    ClusterProperties cp = myParser.getNextSyllable(STRING_L.toCharArray(), 0);
    assertEquals(cp.toString(), 5, cp.getEnd() );
    assertEquals(cp.toString(), MyanmarParser.MyPairStatus.MY_PAIR_EOL,
                 cp.getBreakStatus());
  }
